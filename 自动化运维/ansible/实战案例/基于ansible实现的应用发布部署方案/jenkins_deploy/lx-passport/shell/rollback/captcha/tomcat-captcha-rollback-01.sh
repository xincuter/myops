...
##nginx下线第1批captcha服务,上线第2批captcha服务
NGINX_OFFLINE() {
    $ANSIBLE_CMD "$NGINX_SERVER" -m copy -a "src=$UPSTREAM_FILE dest=/usr/local/nginx/conf.d/upstream/upstream_captcha.conf" &>/dev/null
    if [ $? -eq 0 ];then
