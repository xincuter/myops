#!/bin/bash
#description: account-sync-service应用一键更新脚本...
#version: V1.1
...
##define variables
PROJ_NAME="account-sync-service"
ANSIBLE_CMD="/usr/bin/ansible"
APP_GROUP="prd-lx-passport_account-sync-part02"
UPDATE_SCRIPT="/usr/local/weihu/lx-passport-update_jar-account-sync-service.sh"
NGINX_CONF_DIR="/jenkins_deploy/lx-passport/nginx"
UPSTREAM_FILE="${NGINX_CONF_DIR}/upstream_account-sync.conf-part02"
NGINX_SERVER="10.206.188.23"
LOCAL_PKG_DIR="/jenkins_deploy/lx-passport/sftp/${DATE}/${PROJ_NAME}"
DST_TMP_DIR="/data/update/pkg/account-sync-service_tmp"
UPDATE_LOG_DIR="/jenkins_deploy/lx-passport/logs/update/$PROJ_NAME/part02"
...
##nginx下线第2批account-sync-service服务,上线第1批account-sync-service服务
NGINX_OFFLINE() {
    $ANSIBLE_CMD "$NGINX_SERVER" -m copy -a "src=$UPSTREAM_FILE dest=/usr/local/nginx/conf.d/upstream/upstream_account-sync.conf" &>/dev/null
    if [ $? -eq 0 ];then
