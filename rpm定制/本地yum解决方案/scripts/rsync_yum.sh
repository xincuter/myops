...
##检测本机到网上yum源地址网络是否可达
CHECK_PING() {
    CURL_CODE=$(curl -I -m 5 --retry 3 -o /dev/null -s -w %{http_code} $YUM_URL)       
...
RSYNC_YUM() {
	...
	WRITE_LOG "现在开始同步centos${i} yum源...\n"
        rsync -arvH  --delete --bwlimit=${BW_LIMIT} --exclude="isos" --exclude="*i386" rsync://${YUM_URL}${i} ${LOCAL_DIR} >>$LOG_FILE	
	if [ $? -eq 0 ];then
